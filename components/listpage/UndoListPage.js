...
class UndoListPage extends Component {
      ...
      if (this.undolist.length > 0) {
        ...
        return (
          <View style={{flex:1, flexDirection:'column', alignItems:'center', justifyContent:'center'}}>
            <Text style={{color:'black', fontSize:16, marginBottom: 100}}>太棒了，所有的任务都完成了！</Text>
          </View>
        );
      }
    } else {
  ...
  getUndoItem() {
    ...
    for (var i = 0; i < this.props.todoList.length; i++) {
      if (this.props.todoList[i].status === 'undo') {
...
export default connect(update)(UndoListPage);
